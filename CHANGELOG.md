...
- `rig update [--profile NAME] [--dry-run] [--unattended]` and `rig maintain [--profile NAME] [--dry-run] [--unattended]` accept an unattended mode for a run nobody is watching. Every provider invocation takes its standard input from `/dev/null` and Homebrew is told `NONINTERACTIVE=1`, so a scheduled job can no longer hang forever on a password prompt. Work that needs a person — a Mac App Store upgrade, say — is reported `unavailable` with the reason before its provider is invoked, the rest of the run still completes, and the command returns 1 so the gap stays visible. Selection, order, dispatch, the per-task vocabulary, and the exit statuses are otherwise unchanged.
- An unattended run that dispatched work records what happened in `${XDG_STATE_HOME:-$HOME/.local/state}/rig/last-update`, replaced atomically and never written by a dry run. The tab-separated report opens with `rig-last-run` and its version, carries `action`, `profile`, `platform`, `finished`, `status`, `result`, `detail`, and `summary`, and then the same rows the run printed. It is a stable contract so a wrapper can notify or report without parsing terminal output; Rig itself does not notify, and the report carries no path, locator, argument, credential, or native output. `docs/guides/user/unattended-updates.md` carries the scheduled-job recipe.
