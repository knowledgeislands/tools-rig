...
rig_preflight_lifecycle_task() {
  ...
  case "$binding" in
    skill.*)
      rig_preflight_skill "${binding#skill.}" ||
        rig_fail "skill '${binding#skill.}' update unavailable: ${RIG_SKILL_PREFLIGHT_DETAIL:-preflight-failed}" || return
  ...
  executable=$RIG_VALUE
  rig_executable_available "$executable" ||
    rig_fail "provider '$provider' lifecycle executable unavailable: $executable" || return
