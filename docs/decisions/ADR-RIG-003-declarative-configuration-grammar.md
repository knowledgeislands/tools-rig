...
Rig implements a strict schema-scoped subset of TOML 1.0 directly in Bash. It accepts declared table names, bare schema keys, decimal integer `schema = 1`, single-line basic strings, bounded single-line or multiline arrays of basic strings, blank lines, and `#` comments. Accepted documents are valid TOML. Unsupported TOML types and syntax fail closed.

The declarative model contains categories, one table per tool, composable profiles, services, scheduled jobs, typed settings, semantic Dock layouts and items, publications, optional provider configuration, and explicitly allowed extension actions. Each selectable declaration may contain `profiles`; omission means the configured default profile and an explicit empty array means no profile. A profile may contain `name`, `purpose`, `kind = "complete"` or `kind = "view"`, and explicit `inherits`. A view may inherit only another view. Fixed dotted `install.*` keys keep one installation with its tool. Bounded `variant.ID.*` dotted keys keep mutually exclusive platform installations and artifacts beneath the same tool identity. An `artifacts` array keeps platform-neutral durable generated paths with the capability that owns them and is observation-only. Managed resources may name qualified dependencies, which remain an inert acyclic graph rather than executable hooks.

Schema 1 continues to read central profile member arrays for compatible existing declarations, but one merged configuration cannot combine that selection model with item-owned membership. This fail-closed boundary keeps profile resolution unambiguous while personal configuration moves to the item-owned form.
...
Normal configuration says what belongs and which native authority owns it. Adapter selection, built-in capability discovery, lifecycle sequencing, native command selection, and protocol versioning stay inside Rig. Artifact creation, update, and removal remain with the native tool. External executables remain explicit trust transitions with side effects after parsing.
