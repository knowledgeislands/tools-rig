...
The grammar consists of named sections, `key = literal value` records, blank lines, and whole-line comments. Section types and keys are schema-controlled. Known list fields repeat the key; scalar duplication, unknown fields, conflicting identities, and unsupported schema versions fail closed. The parser splits a record at the first equals sign and performs no quoting, escaping, command evaluation, or general environment expansion. Documented path fields alone expand a leading `~/`.
