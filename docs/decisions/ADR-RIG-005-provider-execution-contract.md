...
Rig builds one work unit for each selected tool with a compatible installation declaration. It orders work dependency-first by `tool.requires`, using bytewise tool identity to break ties. A failed work unit suppresses only its transitive dependants; independent work continues. Catalogue-only tools have no work unit.
...
A provider declaring the exact `inventory` capability returns zero or more stdout lines, each beginning with an identity comparable to an installation `locator`, optionally followed by a tab and opaque provider detail. The separator is a tab rather than a space because an identity may be a filesystem path containing spaces. Inventory is an observation and must not mutate. A non-zero native exit yields state `unknown` with detail `exit:N`; a non-custom adapter or unresolvable executable yields `unavailable`. Rig, not the provider, performs the comparison: it reports every returned identity that no tool installation declares. Installation metadata declares an identity through its `locator`, matched only within its provider namespace because identity spaces collide across providers, or through an `artifacts` array item naming a machine-observable path the tool materialises, matched irrespective of which provider observed it. Artifacts let one provider inventory what another installed and let catalogue-only tools still expose observable materialised state.
...
- [ADR-RIG-003](ADR-RIG-003-declarative-configuration-grammar.md) — defines the inert schema used by providers and tool installations.
