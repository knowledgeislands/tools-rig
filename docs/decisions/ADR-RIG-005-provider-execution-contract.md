...
A generated path remains an artifact of its owning tool rather than becoming a second catalogue identity. Artifact observation is generic and read-only. When creation needs an explicit command, Rig may provide a closed native reconciler that fixes the compatible installation identity, destination, executable, and arguments. The reconciler runs only after successful application or update of its owning tool, participates in preflight, progress, dry-run, verification, and failure reporting, and never becomes a configurable provider or task runner. Removing the tool from a profile does not request package removal or artifact deletion.
