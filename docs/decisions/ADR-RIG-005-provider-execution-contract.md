...
Rig builds dependency-ordered work from the resolved profile. Tool requirements run before dependants; typed settings and layouts, services, and scheduled jobs run only after their required tools. Native-target conflicts are checked across that resolved selection, so mutually exclusive alternatives may coexist in the catalogue while one contradictory profile fails before observation or mutation. A failed work unit suppresses only transitive dependants, while independent work continues. Catalogue-only tools remain valid declarations with no provider work.
...
A generated path remains an artifact of its owning tool rather than becoming a second catalogue identity. Artifact observation is generic and read-only; creation, update, and removal remain the native tool's responsibility. Rig does not treat artifacts as configurable commands or lifecycle hooks. Removing a tool from a profile does not request package removal or artifact deletion.
...
Rig owns planning, preflight, progress, stable reports, failure isolation, and minimal resource reconciliation receipts. Before mutation, each lifecycle plan identifies work as declaration-scoped, manifest-scoped, or provider-wide. Providers retain their native manifests, resolution, execution semantics, credentials, caches, and observed state. Rig does not persist observed installation state. `rig clean` remains a separate Rig-cache command and never cleans provider-native caches; provider maintenance happens only through explicit `rig maintain`.

Receipt-backed reconciliation is serialised per platform target. Rig acquires an exclusive target lock before reading the receipt and holds it through provider work and atomic receipt replacement. A concurrent invocation fails before mutation with its active, stale, or unknown owner diagnostic; it never removes an unverified lock automatically.
