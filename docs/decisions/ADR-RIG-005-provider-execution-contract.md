...
A provider declaring the exact `inventory` capability returns zero or more stdout lines, each beginning with an identity comparable to a binding `locator`, optionally followed by a tab and opaque provider detail. The separator is a tab rather than a space because an identity may be a filesystem path containing spaces. Inventory is an observation and must not mutate. A non-zero native exit yields state `unknown` with detail `exit:N`; a non-custom adapter or unresolvable executable yields `unavailable`. Rig, not the provider, performs the comparison: it reports every returned identity that no binding declares. A binding declares an identity through its `locator`, matched only within that binding's own provider namespace because identity spaces collide across providers, or through a repeatable `artifact` value on its tool naming a machine-observable path that tool materialises, matched irrespective of which provider observed it. Artifacts let one provider inventory what another installed. They are declared on the tool rather than the binding because a tool resolves to exactly one compatible binding per platform, so a second binding cannot carry them, and because a catalogue-only tool has no binding at all yet still materialises observable state.
