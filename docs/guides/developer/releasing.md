...
- [ ] Update `RIG_VERSION` in `src/rig/00-runtime.bash`, assemble `bin/rig`, then confirm the drift check, `rig --version`, changelog release entry, intended `vX.Y.Z` tag, installer examples, and companion Homebrew formula handoff agree.
